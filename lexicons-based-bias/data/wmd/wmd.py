...
raw_data = sys.argv[1]
path_processed_text_data = sys.argv[2]
output = sys.argv[3]
initial_count = int(sys.argv[4])
...
wv = KeyedVectors.load_word2vec_format('./../vectors_generation/wiki_vectors_format_with_comments_without_stopwords.bin', binary=False, unicode_errors="ignore")
wv.init_sims()
...
path_raw = "./../raw_data/"
#raw_data = "raw_noticias_folha_todos_anos_completo.csv"

all_info_text = pd.read_csv(path_raw + raw_data)

path_processed_text = "./../process_text_expressions/"
#path_processed_text_data = "processed_noticias_folha_todos_anos_completo.csv"

print("calculating wmd")

with open(path_processed_text + path_processed_text_data) as f:
    texts=f.readlines()
texts = [x.strip() for x in texts]

lexicons_rates = list()
iter_count = initial_count + 0.
...
write_csv(lexicons_rates, all_info_text)
