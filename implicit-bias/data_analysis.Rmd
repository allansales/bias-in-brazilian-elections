...
# Valid news embeddings
```{r}
threshold = 0.7
embeddings_estadao_news = read_csv("data/2018/noticias_estadao/noticias_eleicao_estadao_config.csv") 
valid_estadao_embeddings = embeddings_estadao_news %>% 
  filter(acc >= threshold) %>% 
  select(id) %>% 
  distinct()

embeddings_folha_news = read_csv("data/2018/noticias_folha/noticias_eleicao_folha_config.csv") 
valid_folha_embeddings = embeddings_folha_news %>% 
  filter(acc >= threshold) %>% 
  select(id) %>% 
  distinct()
```
...
wikipedia_candidates_csv = "biases/2018/candidatos_wikipedia.csv"
wikipedia_party_csv = "biases/2018/partidos_wikipedia.csv"
